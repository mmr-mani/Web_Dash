...
def create_hourly_line_chart(data):
    ...
    fig.update_layout(
        xaxis=dict(
            ...
            ticktext=[str(i) for i in range(0, 25)],
            title='Hour of Day'
        )
    )
...
def create_pie_chart(data,category):
    ...
    fig = px.pie(
        ...
        values='Count',
        title=f"Sentiment Distribution by {category}",
...
merged_raw_data = collect_data("data/Combined_2024-09-16_Analysissheet.xlsx","raw_data")
merged_raw_data['Post_DateTime'] = pd.to_datetime(merged_raw_data['Post_DateTime'])
merged_unique_authors_data = collect_data("data/Combined_2024-09-16_Analysissheet.xlsx",'unique_authors')
...
st.markdown(f"### Like Score:  <span style='color:#f70202'><b>{round(filtered_raw_data['Sentiment_Score'].mean(),4)}</b></span>",unsafe_allow_html=True)
st.markdown(f"### Sentiment Score:  <span style='color:#f70202'><b>{round(filtered_raw_data['Score'].mean(),4)}</b></span>",unsafe_allow_html=True)
...
with tab2:
    # Hour line bar
    posthour_data = filtered_raw_data.groupby(['Post_Hour']).size().reset_index(name='Count')
    fig_positive = create_hourly_line_chart(posthour_data)
...
st.markdown("### @ Hourly Post Counts")
...
with tab1:
    ...
    with col1: 
        ...
        fig_positive = create_hourly_line_chart(positive_sentiment_data)
        st.plotly_chart(fig_positive, use_container_width=True)

    # In the second column, display the dataframe
    with col2:
        st.markdown("#### Top 5 Activity hour with Positive Sentiment submissions")
        st.markdown("######")
...
st.markdown("### @ Top 25 Authors by Submission Counts")
top_25_authors = filtered_unique_authors_data.sort_values(by='Submission_Count', ascending=False).head(25)
...
remove_negative_count = round((negative_authors.shape[0]*percentage/100))


remove_negative_authors= negative_authors.sort_values(by= 'Avg_Sentiment_Score',ascending= True).reset_index(drop=True).head(remove_negative_count)
...
tab1, tab2 = st.tabs(["Submissions", "Authors"])
...
with tab1:
    ...
    with col1:
        ...
        sentiment_scores = pd.DataFrame({
            'Category': ['Raw Data', 'After Removal of Negative Authors'],
